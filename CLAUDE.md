...
│   ├── context_processors.py  # scope (active_household + user_households + forms do modal de lançamento)
...
│   ├── src/modules/      # pageLoader, scopeMenu, passwordToggle, colorSwatches, moneyMask, categoryDonut, categorySelect, filterForm, selectWidget, dateWidget, launchModal, editModal, forecastChart
...
- Fonte: `frontend/src/main.js` (entry) importa os módulos de `frontend/src/modules/` — `pageLoader` (barra de loading), `scopeMenu` (fecha o dropdown de escopo), `passwordToggle` (login), `colorSwatches` (paleta no form de categoria), `moneyMask` (máscara de moeda via `data-money-display`/`data-money-target` — campo Meta de investimento e os valores de lançamento e conta fixa), `categoryDonut` (donut SVG de gastos por categoria no dashboard, lê `#category-data` de um `{% json_script %}`), `categorySelect` (dropdown de categoria dos forms, filtrado por tipo Despesa/Receita e agrupado por natureza Fixa/Variável), `filterForm` (auto-submit de filtros via `data-autosubmit`), `selectWidget` (transforma todo `<select>` nativo em dropdown estilizado), `dateWidget` (widget de data no padrão Emy — `dd/mm/aaaa` + `<input hidden>` ISO + calendário em JS; substitui o `<input type="date">`; ver `_date_field.html`), `launchModal` (modal global de lançamento, desktop, com abas Manual/Conta fixa e submit AJAX), `editModal` (modal de edição por transação na lista de lançamentos, desktop, submit AJAX) e `forecastChart` (gráfico da previsão). Cada módulo é guard-claused, então o bundle único roda em qualquer página.
...
- **Classes de componente, partials e filtro `brl`:** `styles.css` tem um `@layer components` com `.card` e `.btn-primary`, e sombras nomeadas `shadow-card`/`shadow-btn` no `@theme` (em vez de sombras arbitrárias). Markup repetido vive em partials (`_back_button.html`, `_empty_state.html`, `_progress_bar.html`, `_category_select.html`, `_date_field.html`, `_launch_modal.html`, `_edit_modal.html`). Dinheiro usa o filtro `brl` (`finances/templatetags/money.py`): `{{ valor|brl }}` → `R$ 1.234,56` (carregar com `{% load money %}`). Reaproveitar esses padrões ao criar/editar templates. Detalhes em `docs/frontend.md`.
...
- **Estado atual da UI:** identidade **Emy — variação "Petal"** (off-white rosado, soft com glow, cards arredondados, gradiente rosa→roxo), 100% Tailwind + tokens `emy-*` (sem `<style>` inline). O layout é um **app shell** sem scrollbar (página fixa em `100dvh`, só o `main` rola por dentro sem barra); a navegação tem dois menus (seletor de escopo em dropdown no topo + nav inferior de ícones); o **desktop usa 50/50** (`lg:grid-cols-2`) enquanto o mobile fica em coluna única. Ao criar/editar templates, usar classes Tailwind + tokens Emy e seguir esses padrões (ver `docs/frontend.md`, seção "Layout / app shell").
  - **Última leva (ajustes do tester):** valores de lançamento/conta fixa com **máscara de moeda**; inputs de data viraram **widget de data** próprio (`dateWidget`/`_date_field.html`); descrição não corta mais (até 2 linhas); lista de lançamentos com **ordenação Data \| Valor** (agrupa Receitas/Despesas); `Category` ganhou **natureza Fixa/Variável** (grupos fixos, agrupam listagem e select); **Parcelas** virou `<select>`; e o **lançamento virou modal global** (abas Manual/Conta fixa, AJAX, desktop; mobile mantém o form por URL). Ver `docs/frontend.md`, seção "Modal de lançamento".
...
**CategoryNature** — `fixed` (Fixa), `variable` (Variável) — natureza da categoria; grupos fixos do sistema usados por `Category.nature`
...
- `nature` CharField(10) — choices de `CategoryNature`, default `variable` — agrupa em Fixa/Variável (grupos fixos; "pai" na listagem e no select de lançamento). Não entra nas `UniqueConstraint`
...
- **`CategoryForm`** — `ModelForm` de `Category`, campos `name`, `type`, `nature`, `color`, `icon`, `is_active`. Widget `type=color` para `color`; `nature` (Fixa/Variável) é um toggle no template; `icon` é `ImageField` (upload de imagem — o template usa `<input type="file">` e o form/view recebem `request.FILES`). Recebe `user=` e `household=` por kwarg no `__init__`; no `clean()` atribui `instance.user`/`instance.household` (apenas no create, para não trocar o criador num update de grupo) e sobrescreve `_get_validation_exclusions` para **não** excluir `user`/`household` — assim o `full_clean` roda as `UniqueConstraint` de escopo e a categoria duplicada vira erro de form (não `IntegrityError`/500).
- **`TransactionForm`** — `ModelForm` de `Transaction`, campos `description`, `amount`, `date`, `type`, `category`, `payment_method`, `notes`. O `__init__` recebe `user=` e `household=` por kwarg e:
  - filtra o queryset de `category` para mostrar apenas categorias **ativas do escopo ativo** (`Category.objects.in_scope(user, household)`), ordenadas por `nature` para o select agrupar Fixa/Variável;
  - em `clean()`, atribui `self.instance.user` e `self.instance.household` antes de o `Model.clean()` rodar as validações cruzadas.
  - Campo não-model `installments`: `TypedChoiceField` (coerce `int`, opções "À vista (1x)" a "24x", default 1, renderizado como `<select>`). Acima de 1, o `amount` informado é o **total** e a view gera N parcelas mensais. O `clean()` rejeita se o valor for insuficiente (cada parcela precisa de pelo menos `0.01`).
  - O campo `amount` usa **máscara de moeda** no template (`moneyMask`: campo visível formatado + `<input hidden>` com o valor); a `date` usa o **widget de data** (`_date_field.html`).
- **`RecurringTransactionForm`** — `ModelForm` de `RecurringTransaction`, campos `description`, `amount`, `type`, `category`, `payment_method`, `start_date`, `is_active`. Mesmo padrão do `TransactionForm`: recebe `user=`/`household=`, filtra categorias do escopo (ordenadas por natureza) e atribui `user`/`household` no `clean()`. `amount` com máscara de moeda e `start_date` com widget de data.
- **Modal de lançamento (forms no `base.html`):** o context processor `scope` expõe `launch_transaction_form` (com `date` inicial = hoje) e `launch_recurring_form` (não vinculados, no escopo ativo) que alimentam o modal global. As views `transaction_create`/`recurring_create` respondem JSON quando o submit é AJAX (ver Views e `docs/frontend.md`, "Modal de lançamento").
...
| `transaction_list` | `finances:transaction_list` | Lista as transações do mês selecionado (`?month=`) no escopo ativo. **Filtros combináveis** (E): `?type=income\|expense`, `?category=<pk>`, `?payment_method=<valor>`, busca `?q=` (`description__icontains`) e **ordenação `?sort=date\|amount`** (em `amount` agrupa Receitas/Despesas e ordena maior→menor). Calcula o **resumo** (Entrou/Saiu/Saldo) do conjunto filtrado e o `filter_qs` (com o sort) que a navegação por mês preserva. Materializa as contas fixas do mês. |
| `transaction_create` | `finances:transaction_create` | Cria transação no escopo ativo. Responde JSON ao modal (AJAX): `{"ok": true}` no sucesso, `{"ok": false, "errors": …}` (HTTP 400) na validação; fora do AJAX renderiza/redireciona como antes. |
| `transaction_update` | `finances:transaction_update` | Edita transação dentro do escopo (`get_object_or_404(...in_scope...)`). Responde JSON ao modal de edição (AJAX): `{"ok": true}` no sucesso, `{"ok": false, "errors": …}` (HTTP 400) na validação; fora do AJAX renderiza/redireciona como antes. |
...
| `recurring_create/update/delete` | `finances:recurring_*` | CRUD de conta fixa (dentro do escopo); o create materializa o mês corrente e responde JSON ao modal (AJAX), igual ao `transaction_create`. |
...
- `base.html` — **app shell**: `<body>` em `h-[100dvh] flex flex-col overflow-hidden` (a página nunca rola) e `<main>` como única área rolável (`overflow-y-auto no-scrollbar`, conteúdo centralizado por `my-auto`). Header (logo + selo "Beta V0.1" + **seletor de escopo em dropdown** + usuário/Sair); nav inferior flutuante de **ícones** (Início / Lançamentos / **Previsão** / Investir / Categorias / `+`, item ativo via `request.resolver_match`, rótulo só a partir de `sm:`); barra de loading no topo (disparada por cliques/submits); bloco de mensagens; e o **modal global de lançamento** (`_launch_modal.html`, aberto no desktop pelos botões `data-open-launch`). O seletor de escopo é um `<details>` com Pessoal + grupos (`user_households`) + "Criar novo grupo". Desktop usa layout 50/50 (ver Decisões). Estilização 100% Tailwind, sem `<style>` inline.
...
- `finances/transaction_list.html` — layout 50/50: **painel de filtros + resumo** à esquerda (mês, **ordenação Data \| Valor**, resumo Entrou/Saiu/Saldo do período, filtros combináveis de tipo/categoria/forma de pagamento/busca, "Contas fixas", "Limpar filtros") e a lista à direita (selo "2/12"; cabeçalhos Receitas/Despesas quando ordenado por valor; descrição em até 2 linhas). Filtros e ordenação preservam o mês e auto-submetem. No desktop, "Editar" (ou clicar na descrição) abre o **modal de edição** (`_edit_modal.html`) daquela transação; no mobile segue para a página do form.
- `finances/transaction_form.html` — card dividido: toggle Despesa/Receita, valor grande com **máscara de moeda**, **dropdown de categoria** (widget filtrado por tipo e agrupado por natureza), **widget de data**, método de pagamento, **parcelas como `<select>`** (só na criação) e observações.
- `finances/_back_button.html`, `_empty_state.html`, `_progress_bar.html`, `_category_select.html`, `_date_field.html`, `_launch_modal.html`, `_edit_modal.html` — partials reutilizáveis (botão Voltar, estado vazio, barra de progresso, dropdown de categoria, widget de data, modal de lançamento, modal de edição de transação). O `_category_select.html` aceita `categories` + `selected` (usado pelo modal de edição) com fallback ao `form.category`.
- `finances/recurring_list.html` — grid de cards das contas fixas (valor, dia, ativa/pausada) + ações.
- `finances/recurring_form.html` — card dividido de conta fixa (tipo, valor mensal com máscara de moeda, categoria, vencimento com widget de data, método, ativa).
- `finances/category_list.html` — cards de categoria agrupados por **natureza** (seções Fixa/Variável).
- `finances/category_form.html` — formulário de criação/edição de categoria (toggle de tipo, **toggle de natureza Fixa/Variável**, cor, ícone, ativo).
...
- `TEMPLATES['OPTIONS']['context_processors']` inclui `finances.context_processors.scope` (expõe `active_household`, `user_households` e os forms do modal de lançamento `launch_transaction_form`/`launch_recurring_form`).
...
- **Widget de data próprio (não `<input type="date">`)**: os inputs de data viraram um widget no padrão Emy (`dateWidget` + `_date_field.html`) — botão `dd/mm/aaaa`, `<input hidden>` com o valor ISO e um calendário em JS puro. Além de coerência visual, resolve na raiz o bug em que a data sumia na edição: o template localizava a data (`LANGUAGE_CODE` en-us → "June 8, 2026"), inválida para o input nativo; o widget controla o valor sempre em ISO. Os valores de dinheiro do lançamento/conta fixa passaram a usar a `moneyMask` (o `<input hidden>` renderiza com `|unlocalize` para o decimal não localizar e quebrar o parse).
- **Categoria com natureza Fixa/Variável (grupos fixos, não hierarquia)**: pedido de "categoria pai/filha", resolvido com um campo `nature` (`fixed`/`variable`, default `variable`) em vez de um auto-FK `parent`. Os dois grupos são fixos e não editáveis; a listagem e o select de lançamento agrupam por natureza (efeito pai→subcategoria) sem o custo de uma árvore. Nomenclatura distinta de "conta fixa" (`RecurringTransaction`).
- **Ordenação Data | Valor na lista**: a `transaction_list` ganhou `?sort=date|amount`; em `amount` ordena `("-type", "-amount")` — agrupa Receitas e Despesas e ordena maior→menor dentro de cada (cabeçalhos via `{% ifchanged %}`). Default segue por data. O `sort` entra no `filter_qs` para a navegação por mês preservar.
- **Parcelas como `<select>`**: o campo não-model `installments` deixou de ser `<input type=number>` cru e virou `TypedChoiceField` ("À vista (1x)" a 24x), renderizado como `<select>` enriquecido pelo `selectWidget` — coerente com a regra "todo componente de seleção é widget".
- **Lançamento como modal global (desktop) com AJAX**: o lançamento é o form padrão e vive num modal global (`_launch_modal.html` no `base.html`, abas Manual/Conta fixa). Os botões de lançar levam `data-open-launch`: no desktop (≥`lg`) o `launchModal` abre o modal; no mobile o link segue para a página do form (comportamento padrão). O submit é por `fetch` (`X-Requested-With`): `transaction_create`/`recurring_create` respondem `{"ok": true}` (o JS recarrega a página atual e a `messages.success` aparece no reload) ou `{"ok": false, "errors": …}` (HTTP 400 — o JS injeta os erros nos slots `[data-launch-error]` **sem recarregar**, preservando o que foi digitado). Os forms vêm do context processor `scope` (não vinculados, no escopo ativo; o manual já com `date`=hoje); ids prefixados (`id_lm_tx_*`/`id_lm_rt_*`) evitam colisão entre as abas. Escolheu-se recarregar no sucesso (server-rendered, simples) e tratar erro no modal (AJAX) para não perder a navegação.
- **Edição de lançamento como modal (desktop) com AJAX**: a edição na lista de lançamentos espelha o modal de criação — no desktop, "Editar" (ou clicar na descrição) abre o `_edit_modal.html` daquela transação, já preenchido, e submete por AJAX para `transaction_update` (mesmo contrato `{"ok": …}` do create; erros nos slots `[data-launch-error]` análogos via `data-edit-error`); no mobile o link segue para a página do form. Cada modal é **renderizado server-side por transação** dentro do loop da lista (a partir do objeto `t` + as `categories` do escopo já no contexto, sem instanciar um `TransactionForm` por item), porque os widgets (data/categoria/moeda/`selectWidget`) são inicializados uma vez no load e não têm API para repopular — então é mais robusto que injetar HTML por AJAX ou "setar" widgets via JS. Sem abas nem parcelas (não se aplicam à edição); ids prefixados pela pk (`id_em_*_<pk>`) evitam colisão entre os modais. Trade-off conhecido: um modal por transação no DOM (coerente com a lista, que já carrega o mês inteiro sem paginação); se pesar, evoluir para um único modal carregado sob demanda.
